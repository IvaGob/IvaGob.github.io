...
const UserPage = () => {
    ...
    try {
        const idToken = await user.getIdToken(); // Отримуємо ID Token користувача

        const response = await fetch(`${SERVER_URL}/api/order/removeTour`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${idToken}` // передаємо токен в заголовку
            },
            body: JSON.stringify({ tourId: idToRemove }) // тепер передаємо лише tourId
        });
